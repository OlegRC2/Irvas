...
/* harmony import */ var _modules_gallary__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./modules/gallary */ "./src/js/modules/gallary.js");
...
/* harmony import */ var _modules_calc__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./modules/calc */ "./src/js/modules/calc.js");
 ...
 // подключаем файл с модальным окном

 // подключаем файл с табами

 // подключаем файл с галереей

 // подключаем файл с таймером

 // подключаем файл с формами

 // подключаем файл с калькулятором

window.addEventListener('DOMContentLoaded', function () {
  // ждем пока загрузится весь документ 
  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_1__["default"])('.popup_engineer', '.popup_engineer_btn'); // функция модального окна для кнопки "вызвать замерщика"

  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_1__["default"])('.popup', '.phone_link', 60000, '.popup'); // функция модального окна для двух других кнопок на странице, 3-й аргумент время через которое будет открываться окно в 4-м аргументе. Эти 2 аргумента могут не передаваться

  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_1__["default"])('.popup_calc', '.popup_calc_btn'); // функция модального окна с калькулятором

  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.glazing_block', '.glazing_content', '.glazing_slider', 'active', '.first_tabs_block'); // табы для первого блока с табами

  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.decoration_item', '.decoration_content_item', '.decoration_slider', 'after_click', '.second_tabs_block'); // табы для второго блока с табами

  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.balcon_icons_img', '[data-modal-calc-content]', '.balcon_icons', 'do_image_more', '.calc_tabs_block'); // табы для третьего блока с калькулятором

  Object(_modules_gallary__WEBPACK_IMPORTED_MODULE_3__["default"])('.works', '.preview'); // функция для работы галереи на сайте

  Object(_modules_timer__WEBPACK_IMPORTED_MODULE_4__["default"])('.container1', '2020-08-01'); // функция для таймера на странице

  Object(_modules_calc__WEBPACK_IMPORTED_MODULE_6__["default"])();
  Object(_modules_forms__WEBPACK_IMPORTED_MODULE_5__["forms"])('.form'); // функция для отправки форм
});
...
/* harmony import */ var _forms__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./forms */ "./src/js/modules/forms.js");
 ...
 // подключаем функцию для отправки данных на сервер и вывод статуса отправки пользователю

function calc() {
  // функция для калькулятора
  var firstModal = document.querySelector('.popup_calc'),
      // берем первое модальное окно
  secondModal = document.querySelector('.popup_calc_profile'),
      // берем второе модальное окно
  thirdModal = document.querySelector('.popup_calc_end'),
      // берем третье модальное окно
  closeButton = document.querySelectorAll('[data-close]'),
      // берем все кнопки закрытия по дата атрибуту
  calcForm = document.querySelector('[data-form]'); // берем последнее окно с формой в калькуляторе

  var dataCalc = {}; // создаем пустой объект, в нем будет вся итоговая инфа
  ...
  function closeBtn(modalSelector) {
    // функция закрытия окна при клике на кнопку или подложку
    ...
    closeButton.forEach(function (btn) {
      // т.к. получаем псевдомассив, то надо его перебрать и навесить обработчик на каждую кнопку закрытия окна
      btn.addEventListener('click', function () {
        return closeModal(modalSelector);
      });
    });
  }

  function message(modalSelector, messageText) {
    // функция вывода сообщения при неправильных действиях пользователя
  ...
  function typeWindow() {
    // функция работы первого окна калькулятора
    var tabsWindow = document.querySelectorAll('.balcon_icons_img'),
        // берем блок с табами
    width = document.getElementById('width'),
        // берем инпут с шириной
    height = document.getElementById('height'),
        // берем инпут с высотой
    button1Window = document.querySelector('.popup_calc_button'); // берем кнопку "Далее"

    button1Window.addEventListener('click', function () {
      // навешиваем обработчик клика на кнопку
      if (width.value && height.value) {
        // если поля ширина и высота заполнены, то
        tabsWindow.forEach(function (item, i) {
          // перебираем все табы
          if (item.classList.contains('do_image_more')) {
            // если у таба присутствует класс активности, то
        ...
        var messageText = 'Необходимо запонить ширину и высоту'; // пишем текст сообщения
  ...
  function typeGlassWindow() {
    var button2Window = document.querySelector('.popup_calc_profile_button'); // берем кнопку "Далее"

    button2Window.addEventListener('click', function () {
      // навешиваем обработчик клика на кнопку
      var selectForm = document.getElementById('view_type'),
          // берем форму с выбором остекления
      check = document.getElementsByName('checkbox-test'),
          // берем оба чекбокса
      selectId = selectForm.options.selectedIndex; // получаем индекс выбранного остекления

      if (check[0].checked && check[1].checked) {
        // если выбраны сразу 2 чекбокса, то
        var messageText = 'Необходимо выбрать только 1 тип профиля, либо холодное, либо теплое'; // сообщение пользователю
        ...
        // если не выбран ни один чекбокс
        var _messageText = 'Необходимо выбрать тип профиля'; // сообщение пользователю

        message('.popup_calc_profile_content', _messageText); // выводим это сообщение в модальном окне
      } else {
        // если форма заполнена правильно, т.е. выбран профиль и стоит 1 чекбокс
        dataCalc.typeGlass = selectForm.options[selectId].text; // добавляем в итоговый объект выбранное остекление

        if (check[0].checked) {
          // если отмечен первый чекбокс, то
          dataCalc.profile = 'cold';
        } else {
          // если отмечен второй чекбокс, то
  ...
  calcForm.addEventListener('submit', function (e) {
    // навешиваем обработчик события на форму калькулятора, срабатывает при отправке
    e.preventDefault(); // отменяем действия браузера по умолчанию, чтобы не перезагружалась страница

    Object(_forms__WEBPACK_IMPORTED_MODULE_1__["addMessageAndData"])(calcForm, dataCalc); // вызываем функцию отправки данных на сервер и вывод статуса отправки пользователю                                               
  });
}
...
/*!*********************************!*\
  !*** ./src/js/modules/forms.js ***!
  \*********************************/
/*! exports provided: forms, addMessageAndData */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "forms", function() { return forms; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "addMessageAndData", function() { return addMessageAndData; });
...
/* harmony import */ var _services_services__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ../services/services */ "./src/js/services/services.js");
 ...
 // импортируем функции, которые используются в этом коде

function forms(formSelector) {
  // сюда передаем селектор формы
  var forms = document.querySelectorAll(formSelector); // получаем все формы со страницы
  ...
  function bindPostData(form) {
    if (form.dataset.form != 'calc') {
      // дальнейший код не будет выполняться на форме калькулятора, т.к. там отличаются отправляемые данные
      form.addEventListener('submit', function (e) {
        // навешиваем обработчик события на форму, срабатывает при отправке
        e.preventDefault(); // отменяем действия браузера по умолчанию, чтобы не перезагружалась страница
...
function addMessageAndData(form) {
  var calcData = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  // функция показывающая статус и отправляющая на сервер данные, по дефолту данные с калькулятора отключены, но их можно передать, когда данные будут получены 
  var message = {
    // объект для обратной связи пользователю
  ...
  if (calcData && Object.entries(calcData).length != 0) {
    // если данные с калькулятора переданы и этот объект с данными не пустой, то
    for (var key in calcData) {
      // перебираем по ключам объект
  ...
  var json = JSON.stringify(Object.fromEntries(formData.entries())); // сначала преобразуем formData в массив с массивами (entries), затем превращаем в обычный объект (fromEntries), а потом превращаем этот объект в JSON (JSON.stringify)

  Object(_services_services__WEBPACK_IMPORTED_MODULE_8__["postData"])('http://localhost:3000/requests', json) // отправляем данные на сервер
  .then(function (data) {
...
function gallary(gallaryWindowSelector, previewSelector) {
  var gallaryWindow = document.querySelector(gallaryWindowSelector),
  ...
  function closeImg() {
    ...
    }, 900); // удаляем созданный блок через 900 мс после анимации
...
function modals(modalSelector, modalBtn) {
  var time = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 60000;
  var timerModal = arguments.length > 3 ? arguments[3] : undefined;
  var modalTimerId;

  if (modalSelector == timerModal) {
    // если передан класс .popup, то открываем это окно через нужное время           
    modalTimerId = setTimeout(function () {
      return openModal(modalSelector);
    }, time); // открытие модального окна через определенное время
  }

  function openModal(modalSelector) {
    // функция для открытия модального окна
    if (document.body.style.overflow != 'hidden') {
      // если в данный момент окно открыто, то (не открывать еще окно)
      var _modal = document.querySelector(modalSelector); // берем модальное окно


      _modal.style.display = 'block'; // делаем окно видимым
  ...
  function closeModal(modalSelector) {
    // функция для закрытия модального окна
    var modal = document.querySelector(modalSelector); // берем модальное окно
  ...
  var modalButton = document.querySelectorAll(modalBtn),
      // берем все кнопки вызова окна по классу
  modal = document.querySelector(modalSelector),
      // берем само модальное окно
  ...
  modalButton.forEach(function (btn) {
    ...
    btn.addEventListener('click', function (e) {
      ...
      if (modalSelector == timerModal && e.target && e.target.classList.contains(modalBtn.slice(1))) {
        // если в окно, которое должно открываться через n время уже тыкали, то
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass, idClassTabs) {
  ...
  function hideTabContent() {
    ...
    tabs.forEach(function (item) {
      ...
      if (idClassTabs == '.calc_tabs_block') {
        // для табов на калькуляторе
  ...
  tabsParent.addEventListener('click', function (event) {
    ...
    if (target && target.classList.contains(idClassTabs.slice(1))) {
      // смотрим что кликнули в нужный элемент по ТЗ, т.к. класс с точкой, то нужно ее вырезать
      tabs.forEach(function (item, i) {
        // перебираем все табы 
        if (idClassTabs == '.second_tabs_block') {
          // необходимо понять на каком блоке табов выполняется действия, если на втором, то
          if (target.parentElement.parentElement == item) {
            // если родитель родителя кликнутого элемента, совпадает с перебираемым
            hideTabContent(); // скрываем все табы и

            showTabContent(i); // показываем тот, который сейчас в переборе
          }
        } else {
          // если действия выполняются на 1 блоке табов или калькуляторе, то
          if (target.parentElement == item) {
            // если родитель кликнутого элемента, совпадает с перебираемым
            hideTabContent(); // скрываем все табы и
...
var timer = function timer(id, deadLine) {
  function getTimeRemaining(endTime) {
    // функция для расчета времени между временем пользователя и конечным временем таймера
    var t = Date.parse(endTime) - Date.parse(new Date()),
        // переводим дату из формата строки в миллисекунды (Data.parse) и вычитаем текущую дату 
    days = Math.floor(t / (1000 * 60 * 60 * 24)),
        // полученную разницу в миллисекундах делим на кол-во миллисекунд в 1 дне и получаем оставшиеся время в днях
    hours = Math.floor(t / (1000 * 60 * 60) % 24),
        // % возвращает остаток от деления. В данной операции получаем кол-во дней, а остаток это кол-во часов
    minutes = Math.floor(t / 1000 / 60 % 60),
        // % возвращает остаток от деления. В данной операции получаем кол-во часов, а остаток это кол-во минут
  ...
  function getZero(num) {
    // функция для добавления 0 если значение имеет 1 символ. Например часов осталось 9, функция сделает 09
    if (num >= 0 && num < 10) {
      // если приходящее в функцию число больше либо равно 0 и меньше 10
      return "0".concat(num); // возвращаем число с 0 впереди
    } else {
  ...
  function setClock(selector, endTime) {
    // функция для установки рассчитанного времени на страницу
    var timer = document.querySelector(selector),
        days = timer.querySelector('#days'),
        hours = timer.querySelector('#hours'),
        // получили эти элементы со страницы
    ...
    function updateClock() {
      // расчет времени, который остался на текущий момент
      var t = getTimeRemaining(endTime); // получили рассчитанный объект со всеми значениями
      ...
      switch (_context.prev = _context.next) {
        case 0:
          ...
          return regeneratorRuntime.awrap(fetch(url, {
            // делаем запрос к серверу при помощи fetch. await используется в паре с async и это значит, что пока fetch не вернет ответ далеше код не выполнится (ожидание по дефолту 30с)
            method: "POST",
            // прописываем метод запроса
            headers: {
